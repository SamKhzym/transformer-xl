...
def positionwise_FF(inp, d_model, d_inner, dropout, kernel_initializer,
  ...
  with tf.compat.v1.variable_scope(scope):
    ...
    output = tf.compat.v1.layers.dropout(output, dropout, training=is_training,
                               name='drop_2')
    output = tf.compat.v1.keras.layers.LayerNormalization(axis=-1, epsilon=1e-12)(output + inp)
  return output
    ...
    if div_val == 1:
      ...
      for i in range(len(cutoff_ends) - 1):
        with tf.compat.v1.variable_scope('cutoff_{}'.format(i)):
          ...
          y += tf.scatter_nd(mask_idx, cur_y, tf.cast(tf.shape(y), dtype=tf.int64))
      ...
      for i in range(len(cutoff_ends) - 1):
        with tf.compat.v1.variable_scope('cutoff_{}'.format(i)):
          ...
          nll += tf.scatter_nd(mask_idx, -cur_logprob,
                                 tf.cast(tf.shape(nll), dtype=tf.int64))
  if return_mean:
